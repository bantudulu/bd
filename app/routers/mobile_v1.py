...
from app.models import Assignment, FormField, Kategori, Layanan, LayananVarian, Notifikasi, Pesanan, Petugas, User
...
def _session_tokens(user: User, session: MobileSession, refresh_token: str) -> dict:
    access = create_mobile_access_token(
        user_id=user.id,
        email=user.email,
        nama=user.nama,
        role=user.role,
...
async def mobile_login(body: MobileLoginBody, db: AsyncSession = Depends(get_db)):
    ...
    session = MobileSession(
        ...
        device_name=body.device_name.strip() if body.device_name else None,
    ...
    return ok({
        ...
        "session_id": session.id,
        **_session_tokens(user, session, raw_refresh),
    })
...
async def mobile_refresh(body: RefreshBody, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(MobileSession).where(MobileSession.refresh_token_hash == token_hash))
    ...
    # Rotate refresh token on every use so a copied old token cannot be replayed indefinitely.
...
async def services(user=Depends(require_mobile_customer), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Layanan).where(Layanan.aktif == True).order_by(Layanan.nama.asc()))
    rows = result.scalars().all()
    data = []
    for row in rows:
        category = await db.get(Kategori, row.kategori_id)
        variants_result = await db.execute(select(LayananVarian).where(LayananVarian.layanan_id == row.id).order_by(LayananVarian.harga.asc()))
        ...
        data.append({
            ...
            "kategori": {"id": category.id, "nama": category.nama, "slug": category.slug, "icon": category.icon} if category else None,
            "harga_min": min((int(v.harga) for v in variants), default=0),
        })
    return ok(data)
...
async def service_detail(service_id: str, user=Depends(require_mobile_customer), db: AsyncSession = Depends(get_db)):
    ...
    variants_result = await db.execute(select(LayananVarian).where(LayananVarian.layanan_id == service.id))
...
def _order_link(order: Pesanan) -> dict:
    return {
        "deep_link": f"bantudulu://orders/{order.kode}",
        "web_path": f"/pesanan/{order.kode}",
    }


async def _active_assignment_payload(db: AsyncSession, order: Pesanan) -> dict | None:
    ...
    if not assignment:
        return None
    worker = await db.get(Petugas, assignment.petugas_id)
    return {
        "id": assignment.id,
        ...
        "petugas": {
            ...
            "nama": worker.nama,
            "foto": worker.foto,
            "wilayah": worker.wilayah,
        } if worker else None,
    }


async def _order_payload(db: AsyncSession, order: Pesanan) -> dict:
    ...
    return {
        ...
        "kode": order.kode,
        "status": order.status,
        "layanan": {"id": service.id, "nama": service.nama} if service else None,
        "varian": {"id": variant.id, "nama": variant.nama} if variant else None,
        "jadwal": order.jadwal,
        "jam": order.jam,
        "durasi": order.durasi,
        "alamat": order.alamat,
        ...
        "assignment": await _active_assignment_payload(db, order),
        "links": _order_link(order),
...
@router.get("/orders")
async def orders(status: str | None = None, limit: int = 30, user=Depends(require_mobile_customer), db: AsyncSession = Depends(get_db)):
    limit = max(1, min(limit, 100))
    stmt = select(Pesanan).where(Pesanan.user_id == user["sub"])
    if status:
        stmt = stmt.where(Pesanan.status == status.strip().lower())
    result = await db.execute(stmt.order_by(Pesanan.created_at.desc()).limit(limit))
    rows = result.scalars().all()
    return ok([await _order_payload(db, row) for row in rows], meta={"limit": limit, "count": len(rows)})
...
async def order_detail(identifier: str, user=Depends(require_mobile_customer), db: AsyncSession = Depends(get_db)):
    ...
    return ok(await _order_payload(db, order))
...
async def create_mobile_order(
    body: CreatePesananBody,
    request: Request,
    idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"),
    user=Depends(require_mobile_customer),
    ...
    existing_result = await db.execute(
        select(OrderIdempotency).where(
            OrderIdempotency.user_id == user["sub"],
    ...
    if existing:
        ...
        response = ok(await _order_payload(db, order), meta={"idempotent_replay": True})
        return response

    claim = OrderIdempotency(
        user_id=user["sub"],
        idempotency_key=key,
    ...
    try:
        ...
        order_id = legacy_result["data"]["id"]
        order = await db.get(Pesanan, order_id)
        if not order:
            raise RuntimeError("Pesanan berhasil dibuat tetapi tidak dapat dimuat kembali.")
        claim = await db.get(OrderIdempotency, claim.id)
        claim.pesanan_id = order.id
        await db.commit()
        return ok(await _order_payload(db, order), meta={"idempotent_replay": False})
    except Exception:
        # Keep the claim on unknown outcomes to prevent a network retry from creating a duplicate.
        await db.rollback()
        raise


@router.get("/notifications")
async def notifications(limit: int = 50, user=Depends(require_mobile_customer), db: AsyncSession = Depends(get_db)):
    limit = max(1, min(limit, 100))
    result = await db.execute(
        select(Notifikasi)
        .where(Notifikasi.user_id == user["sub"])
    ...
    data = []
    for row in rows:
        order = await db.get(Pesanan, row.pesanan_id) if row.pesanan_id else None
        data.append({
            "id": row.id,
            "judul": row.judul,
            "pesan": row.pesan,
            ...
            "order_code": order.kode if order else None,
            "deep_link": f"bantudulu://orders/{order.kode}" if order else None,
        })
    return ok(data)
...
async def notification_unread_count(user=Depends(require_mobile_customer), db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(func.count(Notifikasi.id)).where(Notifikasi.user_id == user["sub"], Notifikasi.dibaca == False)
    )
...
async def register_mobile_device(body: DeviceBody, user=Depends(require_mobile_customer), db: AsyncSession = Depends(get_db)):
    ...
    if device:
        ...
        device = PushDevice(user_id=user["sub"], token=body.token, platform=platform, aktif=True)
        db.add(device)
    await db.commit()
