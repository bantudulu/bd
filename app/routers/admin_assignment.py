...
@router.get("/petugas")
async def list_petugas(request: Request, aktif: bool | None = None, db: AsyncSession = Depends(get_db)):
...
async def create_petugas(body: PetugasCreate, request: Request, db: AsyncSession = Depends(get_db)):
    require_admin(request)
...
async def update_petugas(petugas_id: str, body: PetugasUpdate, request: Request, db: AsyncSession = Depends(get_db)):
    require_admin(request)
...
async def update_operational_status(pesanan_id: str, body: OperationalStatusUpdate, request: Request, db: AsyncSession = Depends(get_db)):
    require_admin(request)
...
async def get_assignment_history(pesanan_id: str, request: Request, db: AsyncSession = Depends(get_db)):
    require_admin(request)
    order = await _find_order(db, pesanan_id)
    if not order:
        raise HTTPException(status_code=404, detail="Pesanan tidak ditemukan.")
...
async def assign_petugas(pesanan_id: str, body: AssignmentCreate, request: Request, db: AsyncSession = Depends(get_db)):
